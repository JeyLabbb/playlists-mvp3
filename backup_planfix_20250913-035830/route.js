...
function deriveConstraints(promptRaw) {
  ...
  let decade = null;
  const mDec = p.match(/\b(19|20)\d0s\b/);
...
PROMPT=${prompt}`.trim();
    ...
    const out = {
      ...
      queryTerms: Array.isArray(planAI.queryTerms) ? planAI.queryTerms.slice(0, 12) : [],
      ...
      years: Array.isArray(planAI.years) ? planAI.years.slice(0,2) : local.years,
    ...
    // Asegurar queryTerms útiles (nombre + años si los hay)
    const baseTerms = [];
