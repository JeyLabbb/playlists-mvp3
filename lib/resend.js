...
export function getResendClient() {
  ...
  console.log('[RESEND] Inicializando cliente Resend con API key:', apiKey ? `${apiKey.substring(0, 10)}...` : 'NO CONFIGURADO');
...
export async function sendFeedbackEmail(payload) {
  ...
  // Log para debugging
  console.log('[FEEDBACK] 📧 Enviando feedback a:', to);
  console.log('[FEEDBACK] 📧 Desde:', from);
  console.log('[FEEDBACK] 📧 Variables de entorno:', {
  ...
  try {
    console.log('[FEEDBACK] 📤 Intentando enviar email...');
    console.log('[FEEDBACK] 📤 From:', from);
    console.log('[FEEDBACK] 📤 To:', to);
    ...
    console.log('[FEEDBACK] ✅ Respuesta de Resend recibida');
    console.log('[FEEDBACK] ✅ Resultado Resend completo:', JSON.stringify(result, null, 2));
    
    // Resend devuelve el ID en result.data.id o result.id
    const emailId = result?.data?.id || result?.id;
    console.log('[FEEDBACK] ✅ Resend ID:', emailId);
    console.log('[FEEDBACK] ✅ Resend data:', result?.data);
    console.log('[FEEDBACK] ✅ Resend error:', result?.error);
    
    // Verificar si realmente se envió
    if (result?.error) {
      console.error('[FEEDBACK] ❌ Resend devolvió un error:', result.error);
      throw new Error(`Resend error: ${JSON.stringify(result.error)}`);
    }
    
    if (!result || !emailId) {
      console.error('[FEEDBACK] ❌ Resend no devolvió un ID válido. El email NO se envió.');
      console.error('[FEEDBACK] ❌ Resultado completo:', result);
      throw new Error('Resend no devolvió un ID válido');
    }
    
    console.log('[FEEDBACK] ✅ Email enviado correctamente a:', to);
    console.log('[FEEDBACK] ✅ Email ID de Resend:', emailId);
    
    // Advertencia sobre onboarding@resend.dev
    if (from.includes('onboarding@resend.dev')) {
      console.warn('[FEEDBACK] ⚠️ ADVERTENCIA: Estás usando onboarding@resend.dev');
      console.warn('[FEEDBACK] ⚠️ Este dominio solo puede enviar a emails VERIFICADOS en tu cuenta de Resend');
      console.warn('[FEEDBACK] ⚠️ Ve a https://resend.com/emails y verifica que', to, 'esté en la lista de emails permitidos');
    }
    
    return result;
  } catch (error) {
    console.error('[FEEDBACK] ❌ Error enviando feedback a:', to);
    console.error('[FEEDBACK] ❌ Error completo:', error);
    console.error('[FEEDBACK] ❌ Error message:', error?.message);
    console.error('[FEEDBACK] ❌ Error name:', error?.name);
    console.error('[FEEDBACK] ❌ Error response:', error?.response);
    console.error('[FEEDBACK] ❌ Stack:', error?.stack);
    
    // Si es un error de Resend, mostrar más detalles
    if (error?.response) {
      console.error('[FEEDBACK] ❌ Resend response status:', error.response?.status);
      console.error('[FEEDBACK] ❌ Resend response data:', JSON.stringify(error.response?.data, null, 2));
    }
