...
export function extractFestivalInfo(prompt) {
  ...
  // FIXPACK: Extraer año del prompt
  ...
  const words = cleanPrompt.split(/\s+/).filter(word => 
    ...
    !word.match(/^\d{4}$/) // No incluir años
  );
...
function generateFestivalQueries(festivalName, year) {
  ...
  // FIXPACK: Agregar variantes con años cercanos
  ...
  const yearVariantQueries = yearVariants.flatMap(y => [
    `${festivalName} ${y}`,
    `${festivalName} lineup ${y}`,
    `${festivalName} cartel ${y}`
  ]);
  
  return [...baseQueries, ...noYearQueries, ...yearVariantQueries];
}
