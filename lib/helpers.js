...
export function calculateDynamicCaps(targetTracks, distinctPriority = 0, specialCases = {}, hasOnlyArtists = false) {
  ...
  if (distinctPriority === 0) {
    // Cap dinámico entre 5-9 según targetTracks
    // Más tracks = cap más alto, pero siempre priorizando variedad
    let nonPriorityCap;
    if (targetTracks <= 30) {
      nonPriorityCap = 5;
    } else if (targetTracks <= 50) {
      nonPriorityCap = 6;
    } else if (targetTracks <= 80) {
      nonPriorityCap = 7;
    } else if (targetTracks <= 100) {
      nonPriorityCap = 8;
    } else {
      nonPriorityCap = 9;
    }
  ...
  // Calcular cap base para priority (normal x 2)
  let baseNonPriorityCap;
  if (targetTracks <= 30) {
    baseNonPriorityCap = 5;
  } else if (targetTracks <= 50) {
    baseNonPriorityCap = 6;
  } else if (targetTracks <= 80) {
    baseNonPriorityCap = 7;
  } else if (targetTracks <= 100) {
    baseNonPriorityCap = 8;
  } else {
...
export function calculateMultiArtistDistribution(targetTracks, distinctPriority, dynamicCaps = null, specialCases = {}) {
  ...
  if (distinctPriority > 0) {
    ...
    const maxPriorityShare = distinctPriority <= 3 
      ...
      : Math.floor(targetTracks * 0.6); // Máximo 60% para priority si hay muchos
    const minPerPriority = 3; // Mínimo para que todos tengan algo
