...
export async function resolveTracksBySearch(accessToken, llmTracks, options = {}) {
  ...
  const processTrack = async (track) => {
    ...
    for (const query of queries) {
      ...
      // 🚨 VALIDACIÓN: Detectar nombres genéricos que requieren match estricto de artista
      const isGenericName = name.length <= 5 || /^(intro|outro|interlude|prelude|theme|song|track|music)$/i.test(name);
      ...
      for (const spotifyTrack of tracks) {
        ...
        // 🚨 VALIDACIÓN CRÍTICA: Si hay artista en el LLM track, debe coincidir bien
        if (firstArtist) {
          // Si el artista no coincide bien (score < 0.6), rechazar incluso si el nombre coincide
          if (artistScore < 0.6) {
            console.log(`[LLM-RESOLVE] 🚫 REJECTED: "${name}" by ${firstArtist} → "${trackName}" by ${trackArtists[0]} (artistScore: ${artistScore.toFixed(2)} < 0.6)`);
            continue; // Skip this track - artista no coincide
          }
          
          // Si es un nombre genérico, requerir match casi perfecto del artista
          if (isGenericName && artistScore < 0.85) {
            console.log(`[LLM-RESOLVE] 🚫 REJECTED (generic name): "${name}" by ${firstArtist} → "${trackName}" by ${trackArtists[0]} (artistScore: ${artistScore.toFixed(2)} < 0.85)`);
            continue; // Skip - nombre genérico requiere artista casi exacto
          }
        }
        
        // Calcular score total con pesos ajustados
        // Si hay artista, darle más peso (60% artista, 40% nombre)
        // Si no hay artista, usar 70% nombre, 30% artista (default)
        const totalScore = firstArtist 
          ? (nameScore * 0.4) + (artistScore * 0.6)  // Más peso al artista si está presente
          : (nameScore * 0.7) + (artistScore * 0.3);  // Más peso al nombre si no hay artista
        
        // 🚨 UMBRAL MÁS ESTRICTO: Aumentar de 0.7 a 0.85 para evitar matches incorrectos
        // Si es nombre genérico, requerir score aún más alto
        const minScore = isGenericName ? 0.9 : 0.85;
        
        if (totalScore > bestScore && totalScore >= minScore) {
          bestMatch = spotifyTrack;
          bestScore = totalScore;
        } else if (totalScore > bestScore) {
          console.log(`[LLM-RESOLVE] 🚫 REJECTED (low score): "${name}"${firstArtist ? ` by ${firstArtist}` : ''} → "${trackName}" by ${trackArtists[0]} (totalScore: ${totalScore.toFixed(2)} < ${minScore.toFixed(2)})`);
        }
      }
      ...
      if (bestMatch) {
        ...
        console.log(`[LLM-RESOLVE] ✅ Resolved: "${name}"${firstArtist ? ` by ${firstArtist}` : ''} → "${resolved.name}" by ${resolved.artistNames.join(', ')} (score: ${bestScore.toFixed(2)})`);
        return resolved;
      }
    }
