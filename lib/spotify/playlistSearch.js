...
import { FEATURE_COMPOSITE_NAME_SEARCH } from '../features';
 ...
 */
export async function searchFestivalLikePlaylists({
  accessToken,
  baseQuery,
  year,
  searchTerms = null,
  ...
  try {
    ...
    if (Array.isArray(searchTerms) && searchTerms.length) {
      ...
      if (normalizedTerms.length) {
        filtered = filtered.filter(p => {
          const text = norm(`${p.name || ''} ${p.description || ''}`);
          return normalizedTerms.every(term => text.includes(term));
        });
      }
    }

    // 🚨 MEJORA: Fallback más inteligente - solo ignorar año si realmente no hay opciones
    // Primero intentar sin followers pero manteniendo año
    if (filtered.length === 0 && yearStr) {
      filtered = safe.filter(p => {
        const title = norm(p.name || '');
        const desc  = norm(p.description || '');
        const text  = `${title} ${desc}`;
        return text.includes(nq) && text.includes(yearStr) && p.id;
      });
      console.log(`[PL-SEARCH] Fallback (with year, no followers): ${filtered.length} playlists`);
    }
    
    // Si aún está vacío, solo entonces ignorar año (último recurso)
    if (filtered.length === 0) {
      filtered = safe.filter(p => p?.id && norm(p.name||'').includes(nq));
      console.log(`[PL-SEARCH] Fallback (no year requirement): ${filtered.length} playlists`);
      // si aún así 0, usa las primeras por si el nombre no coincide exactamente
      if (filtered.length === 0) {
        filtered = safe.filter(p => p?.id).slice(0, 20);
        console.log(`[PL-SEARCH] Final fallback (any playlist): ${filtered.length} playlists`);
      }
    }
    ...
    if (!unique.length && FEATURE_COMPOSITE_NAME_SEARCH && compositeDepth === 0) {
      const reduced = reduceCompositeLabel(`${baseQuery} ${year || ''}`.trim());
      if (reduced?.reduced && reduced.reduced !== baseQuery) {
        console.log('[PL-SEARCH] Composite fallback triggered', {
          from: baseQuery,
          year,
          to: reduced.reduced,
          descriptor: reduced.descriptor,
        });
        return await searchFestivalLikePlaylists({
          accessToken,
          baseQuery: reduced.reduced,
          year: null,
          searchTerms,
...
function reduceCompositeLabel(label) {
  ...
  while (tokens.length > 1) {
    ...
    if (isDescriptorToken(last)) {
      removed.unshift(tokens.pop());
      continue;
    }
    break;
  }
  ...
  return {
    reduced: tokens.join(' ').trim(),
    descriptor: removed.join(' '),
  };
}
...
function isDescriptorToken(token = '') {
  ...
  if (/^\d{2,4}$/.test(normalized)) return true;
  if (/^(edition|edicion|edicionv|ediccion|season|temporada|vol|volume|volumen)$/.test(normalized)) return true;
  if (/^(i|ii|iii|iv|v|vi|vii|viii|ix|x|xi|xii|xiii|xiv|xv|xvi|xvii|xviii|xix|xx)$/.test(normalized)) return true;
  return false;
}
