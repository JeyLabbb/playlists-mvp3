...
// nombres típicos de festivales (añade aquí lo que quieras)
...
function inferGenres(prompt){
  ...
  for(const g of FALLBACK_GENRES){
    if(p.includes(norm(g))) out.add(g);
  }
  // casos comunes
...
function inferRules(prompt){
  ...
  // bpm sueltos tipo "120-140 bpm"
  const m = prompt.match(/\b(\d{2,3})\s*-\s*(\d{2,3})\s*bpm\b/i);
  if(m){ rules.bpmMin = +m[1]; rules.bpmMax = +m[2]; }

  // heurísticos por palabras
  if(/(nightcore|hardstyle|techno)/i.test(prompt)) rules.bpmMin = rules.bpmMin ?? 120;

  return rules;
}

function detectYears(prompt){
  return (prompt.match(/\b(20\d{2})\b/g) || []).slice(0,2);
}

function detectEvent(prompt){
  const p = norm(prompt);
  if(/(festival|cartel|line[\s-]?up|lineup)/i.test(prompt)) return true;
  for(const k of FESTIVAL_HINTS){
    if(p.includes(norm(k))) return true;
  }
  // si el prompt es corto (1–3 palabras) y no es un género conocido, trátalo como posible evento
...
function makeHeuristicPlan(prompt, count){
  ...
  return {
    candidates: [],               // el recolector /api/recommend buscará
    queryTerms,
    targetGenres,
    isEvent,
    years,
...
- Si el prompt es sólo el nombre de un evento/festival, márcalo como "isEvent": true y rellena "years" si aparecen.
...
PROMPT=${prompt}`.trim();
    ...
    // Último recurso: heurística para NO romper el front
