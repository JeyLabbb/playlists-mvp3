    ...
    try {
      ...
      const getOrigin = () => {
        ...
        if (isLocalhost && !isVercelPreview) {
          console.log('[REGISTER] ✅ Detected LOCAL development, using:', origin);
          return origin;
        }
        
        // Regla 2: Si es dominio de producción → producción
        if (isProductionDomain) {
          console.log('[REGISTER] ✅ Detected PRODUCTION domain, using:', origin);
          return origin;
        }
        ...
        if (isVercelPreview) {
          ...
          console.log('[REGISTER] ✅ Detected VERCEL PREVIEW, using production URL:', productionUrl);
          return productionUrl;
        }
        
        // Fallback: usar producción
        const productionUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://playlists.jeylabbb.com';
        console.log('[REGISTER] ✅ Fallback to production URL:', productionUrl);
        return productionUrl;
      };
      
      const callbackUrl = `${getOrigin()}/auth/callback?redirect=${encodeURIComponent(redirectTo || '/')}`;
      console.log('[REGISTER] 📋 Callback URL construida:', callbackUrl);
